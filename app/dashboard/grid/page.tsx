...
export default function GridBotPage() {
  ...
  const fetchBots = async () => {
    ...
    try {
      ...
      const gridBots = (response.data || []).filter((bot: BotConfigResponse) => bot.bot_type === 'grid');
      setBots(gridBots);
    } catch (error: any) {
  ...
  const handleStartStop = async (bot: BotConfigResponse, action: 'start' | 'stop') => {
     ...
     setError(null);
     // Consider adding specific loading state per button/row
     try {
        const { data: { session }, error: sessionError } = await supabase.auth.getSession();
        if (sessionError) throw new Error(`Supabase auth error: ${sessionError.message}`);
        const token = session?.access_token;
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold mb-6">Grid Trading Bot</h1>
      
      {/* List Existing Bots */}
      <div className="mb-8">
        <h2 className="text-lg font-medium mb-3">Your Grid Bots</h2>
        {isLoading && <p>Loading bots...</p>}
        {error && <p className="text-destructive">Error loading bots: {error}</p>}
        {!isLoading && !error && bots.length === 0 && (
          <p className="text-muted-foreground">You haven't created any Grid bots yet.</p>
        )}
        {!isLoading && !error && bots.length > 0 && (
